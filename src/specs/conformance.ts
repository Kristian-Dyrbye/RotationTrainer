...
import { buildIdOf } from './index'

const STATS = { haste: 0.15, critChance: 0.2, critMult: 2.0 }
const CFG = { seed: 77, duration: 120, stats: STATS }

/**
 * The shared conformance battery every spec build must pass.
 * Registered builds run it via allSpecs.test.ts; a build's own
 * <buildId>.test.ts can run it before registration.
 */
export function conformanceSuite(spec: SpecConfig): void {
  describe(`${spec.name} [${buildIdOf(spec)}] conformance`, () => {
