 ...
 * Breath of Sindragosa dual-wield raid ST build (alternate to the default
 * Rider build; Maxroll/Icy Veins put it ~1% behind Rider on raid ST).
 * Verified 2026-08-24 against Wowhead's rotation guide (via search
 * snippets), Icy Veins (rotation + builds pages), Method (which publishes
 * the Deathbringer priority and opener), and Maxroll's 12.1 raid guide.
 *
 * Sources:
 ...
 * for the Rider "Frost Single Target" and "Frost Mythic+/Delves" builds
 * (verified in the raw HTML 2026-08-24), so none is quoted here.
 *
 * Deathbringer kit, per the 12.1 guides: Reaper's Mark (2 runes, 45s CD,
 * "macro it with your Pillar of Frost every time") deals instant damage,
 * then counts your Frost/Shadow damage events as stacks; after 12s or at
 * 40 stacks it detonates for damage per stack. The detonation grants two
 * Exterminate procs: each gives a Killing Machine proc and discounts the
 * next Obliterate by 1 rune while empowering it with a scythe hit. Grim
 * Reaper grants Killing Machine when Reaper's Mark is cast. Obliteration
 * is NOT taken (unlike the Rider build) — no KM fishing during Pillar.
 * Shared with the default build: Breath of Sindragosa, Shattering Blade,
 * Icy Onslaught, Icy Death Torrent, Frozen Dominion (Pillar auto-casts a
 * free Remorseless Winter; not castable on its own), Apex: Chosen of the
 * Frostbrood. S2 tier ("Freezing Tempest"): 2pc — each Remorseless Winter
 * tick grants +2% attack speed and +4% Icy Death Torrent damage, stacking;
 * 4pc — RW ticks 25% more often and the buff lasts 5s longer (10s total).
 *
 ...
 * and per-stack damage, Exterminate scythe coefficient, Breath drain fixed
 * at 16 RP/s. Damage in AP units.
 */
...
const RM_INITIAL = 2.34        // Reaper's Mark on-apply hit
const RM_PER_STACK = 0.30364   // APPROX detonation damage per stack
const EXTERMINATE_COEFF = 2.1  // APPROX empowered-Obliterate scythe hit
...
/** frost damage with Razorice vulnerability; feeds Reaper's Mark stacks */
...
export const frostDKDeathbringer: SpecConfig = {
  ...
  auras: [
    ...
    {
      // Exterminate: Reaper's Mark detonation empowers the next 2 Obliterates
      // (-1 rune, bonus scythe hit); each proc also granted a Killing Machine
      id: 'exterminate', name: 'Exterminate', icon: 'ability_revendreth_deathknight', duration: 30, maxStacks: 2,
    },
    {
      ...
      onExpire: (s, aura) => {
        ...
        // detonation grants two Exterminate procs, each with a KM proc
        s.applyAura('player', 'exterminate', { stacks: 2 })
  ...
  abilities: [
    {
      // Exterminate empowers this: -1 rune, bonus scythe hit (Deathbringer)
      id: 'obliterate',
      name: 'Obliterate',
      icon: 'spell_deathknight_classicon',
      spellId: 49020,
      runeCost: 2,
      runeCostMod: (s) => (s.stacks('player', 'exterminate') > 0 ? 1 : 2),
      displayName: (s) => (s.stacks('player', 'exterminate') > 0 ? 'Exterminate' : 'Obliterate'),
      displayIcon: (s) => (s.stacks('player', 'exterminate') > 0 ? 'ability_revendreth_deathknight' : 'spell_deathknight_classicon'),
      displayStacks: (s) => s.stacks('player', 'exterminate'),
      onResolve: (s) => {
        const exterminate = s.stacks('player', 'exterminate') > 0
        s.gain(exterminate ? 10 : 20, 'obliterate')
        if (km(s) > 0) {
        ...
        if (exterminate) {
          s.consumeStack('player', 'exterminate')
          frostDamage(s, 'Exterminate', EXTERMINATE_COEFF)
        }
        addRazorice(s)
      },
    },
    ...
    {
      // "macro it with your Pillar of Frost every time" — Icy Veins
      id: 'reapers_mark',
      name: "Reaper's Mark",
      ...
      cooldown: 45,
      onResolve: (s) => {
        s.gain(20, 'reapers_mark')
        s.damage('reapers_mark', RM_INITIAL, { tags: ['frost'] })
        s.applyAura('target', 'reapers_mark')
        // Grim Reaper: casting Reaper's Mark grants Killing Machine
    ...
    {
      // Apex: recastable within 45s at 50% via the recall buff
      id: 'frostwyrms_fury',
  ...
  actionBar: [
    'obliterate', 'frost_strike', 'howling_blast', 'reapers_mark',
    'pillar_of_frost', 'breath_of_sindragosa', 'empower_rune_weapon',
    'frostwyrms_fury',
  ],
  ...
  glows: (s, id) => {
    switch (id) {
      case 'obliterate': return km(s) > 0 || s.stacks('player', 'exterminate') > 0
  ...
  priorityList: [
    ...
    { abilityId: 'reapers_mark', text: 'With every Pillar — detonation grants 2 Exterminates + 2 Killing Machines (Grim Reaper: the cast itself grants KM)', when: s => s.cooldownRemains('reapers_mark') === 0 },
    ...
    { abilityId: 'frostwyrms_fury', text: 'With Pillar, after the Mark detonates; press again once the 15% haste buff ends', when: s => s.cooldownRemains('frostwyrms_fury') === 0 || s.auraRemains('player', 'fwf_recall') > 0 },
    { abilityId: 'obliterate', text: 'Spend Exterminate charges (1 rune, empowered scythe hit)', label: 'Exterminate', icon: 'ability_revendreth_deathknight', when: s => s.stacks('player', 'exterminate') > 0 },
    { abilityId: 'obliterate', text: 'At 2 Killing Machine always; at 1 KM with 3+ runes' },
    ...
    { abilityId: 'frost_strike', text: 'Never cap Runic Power (deficit <25), or at ≤1 rune' },
    { abilityId: 'obliterate', text: 'With Killing Machine (save 2 runes when Reaper\'s Mark is coming)' },
    ...
    { abilityId: 'obliterate', text: 'No KM — last resort' },
  ],
  ...
  policy: (s) => {
    ...
    // hold 2 runes for Reaper's Mark when it lines up with Pillar
    ...
    // single target core (Method/Icy Veins Deathbringer order)
    if (s.stacks('player', 'exterminate') > 0 && s.isUsable('obliterate') === true) return 'obliterate'
    ...
    if ((100 - rp < 25 || runes <= 1) && !bosUp && !rpPooling && s.isUsable('frost_strike') === true) return 'frost_strike'
